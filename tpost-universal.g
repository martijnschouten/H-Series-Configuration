...
if #tools[{state.nextTool}].name == 5 ; If this tool is a probe... (5 letters)
    M453 ; Switch to CNC mode
    M574 Z1 S2 ; Set Z endstop position to low end and configure as Z probe

elif #tools[{state.nextTool}].name == 7 ; If this tool is called "No Tool"... (7 letters)
    M451 ; Switch to FFF Mode

elif #tools[{state.nextTool}].name == 8 ; If this tool is an "extruder" (8 letters)
    M451 ; Switch to FFF Mode

elif #tools[{state.nextTool}].name == 14 ; If this tool is an "paste extruder" (14 letters)
    M451 ; Switch to FFF Mode

elif #tools[{state.nextTool}].name == 9 ; If this tool is a spindle... (9 letters???)
    M453 ; Switch to CNC mode
...
if state.previousTool != -1 ; If we changed to this tool from another tool...
    ...
    if state.restorePoints[2].coords[2] + 2 <= {move.axes[2].max + min(tools[1].offsets[2], tools[2].offsets[2], tools[3].offsets[2], tools[4].offsets[2], tools[6].offsets[2], tools[10].offsets[2])}
